...
function Restaurant() {
  ...
  let getRestaurantDetails = async () => {
    let url = "http://localhost:4000/api/get-restaurant-details/" + id;
    let { data } = await axios.get(url);
    setRDetails(data.result);
  };
  let getMenuItemList = async () => {
    let url = "http://localhost:4000/api/get-menu-item-list/" + id;
    let { data } = await axios.get(url);
  ...
  let getPaymentView = async () => {
    const url = "http://localhost:4000/api/create-order";
    ...
    let options = {
      ...
      handler: async function (response) {
        ...
        let url = "http://localhost:4000/api/verify-payment";
